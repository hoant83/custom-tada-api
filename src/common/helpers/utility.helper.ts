...
export const csvFilter = (req: any, file: any, callback: any): any => {
...
export const multerOptionsLogo = {
  fileFilter: imageFileFilter,
  limits: { fileSize: 3000000 },
  storage: diskStorage({
    destination: './uploads',
...
export const formatPhone = (phoneNumber: string): string => {
  ...
  if (process.env.REGION.toLocaleLowerCase() === COUNTRY.VIETNAM) {
...
// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
export const removeIgnoredAttributes = (object: any) => {
  const clone = { ...object };
...
// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
export const addBodyToRequest = (
  request: Request,
  body: any,
  customKey?: string,
) => {
  (request as any).customBody = removeIgnoredAttributes(body);
  if (customKey) {
    (request as any).customKey = customKey;
  }
};
...
export const prepareLikeSqlParam = (string: string): string => `%${string}%`;
...
export const roundTwoFixed = (number: number): number => {
